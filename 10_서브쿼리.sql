...
-- 단일 행 연산자 ㅣ 주로 비교 연산자( = , > , <, >=, <= <>)를 사용하는 경우 하나의 행만 반환해야 합니다.
...
--                250 -> false, 200 보다는 크지만 300, 400 보다는 크지 않으므로,
...
a. 간단한 상황에서 사용하면 쿼리가 직관적이고 간곃합니다.
b. 단일 값을 반환하는 서브쿼리에서 유용
c. 대량 데이터가 아닌 경우나, 서브쿼리의 복잡도가 낮은 경우 적합.

1. 대량 데이터를 처리하거나, 여러 칼럼을 붙여아 할떄 유리합니다.
2. 여러 테이블을 한 버넹 조인해야 할 경우.
...
-- from절 서브쿼리(인라인 뷰)
-- 특정 테이블전체가 아닌  select를 통해 일부 데이터를 
-- 조회한 것을 가상 테이브로 사용하고 싶을때,
